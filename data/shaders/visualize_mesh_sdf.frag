...
layout(std140, binding = 2) buffer MeshSignDistanceFieldData
{
    ...
    mat4 world_to_volume[GLOBAL_SDF_MAX_OBJECT_COUNT];
    vec4 volume_to_uvw_mul[GLOBAL_SDF_MAX_OBJECT_COUNT];
    vec4 volume_to_uvw_add[GLOBAL_SDF_MAX_OBJECT_COUNT];
    float resolution;
...
{
    vec3 volume_pos = (mesh_sdf_data.world_to_volume[sdf_index] * vec4(world_position, 1.0)).xyz;
    vec3 volume_uv = volume_pos * mesh_sdf_data.volume_to_uvw_mul[sdf_index].xyz + mesh_sdf_data.volume_to_uvw_add[sdf_index].xyz;
    return volume_uv;
}
